...
supFTSVD_JM <- function(datlist, response=NULL, interval = NULL, r = 3,
  ...
  if(constant_hazard){
    hzr<-as.numeric(coef(lm(bz_hat~ev_time-1)))
...
#' 
#' @export
coxph_mle_fllik<-function(surv_time,censor_status,Xmat,max_iter=25,cnv_crit=1e-6){
  n<-length(surv_time)
  p<-ncol(Xmat)
  ...
  repeat{
    ...
    surv_scr<-hadamard.prod(Xmat,outer(event_status,rep(1,p)))-(uexpRW*fitbz_dat[,1])
    ...
    sllik<-sl_lik(par = sg_par,Xmat = Xmat,surv_time = surv_time,censor_status = censor_status,bz_hat = bz_hat[survPOS],cum_bzhat = fitbz_dat[,1])
    
    iter_dif<-rbind(iter_dif,c(dif,sllik))
    
    if(iter>max_iter | max(dif)<=cnv_crit)
      break
    iter<-iter+1
  }
  
  list("CoxPH_obj"=list("hazard"=bz_hat,
                        "cum_hazard"=fitbz,
                        "coefficients"=sg_par),
       "cnvrgn" = iter_dif)
}
